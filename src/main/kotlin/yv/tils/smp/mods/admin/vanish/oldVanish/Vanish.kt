...
@Deprecated("Old Vanish System", ReplaceWith("Vanish", "yv.tils.smp.mods.admin.vanish.Vanish"), DeprecationLevel.ERROR)
class Vanish {
    companion object {
    ...
    private fun quickVanish(player: Player, sender: CommandSender) {
        ...
        if (vanish[player.uniqueId]!!) {
            VBuilder().vanishRegister(player, false)
            disableVanish(player)
        } else {
            VBuilder().vanishRegister(player, true)
            enableVanish(player)
        }
