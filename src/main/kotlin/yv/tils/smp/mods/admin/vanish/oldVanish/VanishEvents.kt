...
@Deprecated("Old Vanish System", ReplaceWith("VanishEvents", "yv.tils.smp.mods.admin.vanish.VanishEvents"), DeprecationLevel.ERROR)
class VanishEvents {
    fun onRejoin(e: PlayerJoinEvent) {
        ...
        if (Vanish.vanish[player.uniqueId]!!) {
            yv.tils.smp.mods.admin.vanish.Vanish().enableVanish(player)

            yv.tils.smp.mods.admin.vanish.Vanish().playerHide(player)
        } else {
            yv.tils.smp.mods.admin.vanish.Vanish().disableVanish(player)
        }
    }
